 ...
 * @author james
 */
@WebServlet(name = "ReporteCajero", urlPatterns = {"/ReporteCajero"})
public class ReporteCajero extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            ArrayList<Transaccion> transaccion = new ArrayList<Transaccion>();
                ...
                transaccion = gestor.Reporte1(USER);

                if ((transaccion != null)) {
                    request.setAttribute("Transaccion", transaccion);

                    request.getRequestDispatcher("/PagesCajeroReporte/Reporte1.jsp").forward(request, response);
                } else {
                    request.getRequestDispatcher("/PagesCajeroReporte/Reporte1.jsp").forward(request, response);
                }  
            ...
            //request.getRequestDispatcher("/PagesGerenteReporte/Reporte1.jsp").forward(request, response);
        }
    }
