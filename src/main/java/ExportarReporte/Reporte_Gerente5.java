 ...
 * @author James
 */
@WebServlet(name = "Reporte_Gerente5", urlPatterns = {"/Reporte_Gerente5"})
public class Reporte_Gerente5 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            request.setAttribute("s", fecha2);
                ...
                cambios = gestor.reporte5(fecha1, fecha2);

                if ((cambios != null)) {
                    request.setAttribute("Cliente", cambios);

                    request.getRequestDispatcher("/PagesGerenteReporte/Reporte5.jsp").forward(request, response);
                } else {
                    request.getRequestDispatcher("/PagesGerenteReporte/Reporte5.jsp").forward(request, response);
                }  
